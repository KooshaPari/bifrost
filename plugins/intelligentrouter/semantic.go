...
func (ir *IntelligentRouter) extractFeatures(req *schemas.BifrostRequest) FeatureSet {
	...
	if req.Params != nil {
		if tools, ok := req.Params["tools"]; ok && tools != nil {
			features.HasTools = true
		}
		if toolChoice, ok := req.Params["tool_choice"]; ok && toolChoice != nil {
			features.HasToolChoice = true
		}
		if functions, ok := req.Params["functions"]; ok && functions != nil {
	...
	if req.Messages != nil {
		features.MessageCount = len(req.Messages)
